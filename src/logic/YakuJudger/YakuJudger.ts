...
export class YakuJudger {
	...
	private checkYakuman(parsedHand: ParsedHand, hand: Hand): Yaku[] {
		...
		if (this.daisangenChecker.check(parsedHand))
			results.push(YAKU_LIST.DAISANGEN);
		...
		if (this.suukantsuChecker.check(parsedHand))
			results.push(YAKU_LIST.SUUKANTSU);
	...
	private resolveConflicts(yakuList: Yaku[]): Yaku[] {
		...
		for (const pair of conflictPairList) {
			finalYakuList = this.applyConflictRule(
				finalYakuList,
				yakuIds,
				pair.higher,
				pair.lower,
	...
	private applyConflictRule(
		yakuList: Yaku[],
		yakuIds: Set<string>,
		higherYakuId: string,
		lowerYakuId: string,
	): Yaku[] {
		if (yakuIds.has(higherYakuId))
			return yakuList.filter((yaku) => yaku.id !== lowerYakuId);
	...
	private getNextDoraTile(indicator: Tile): Tile {
		...
		const rank = parseInt(indicator.charAt(0), 10);
