...
export class ChantaChecker {
	public checkChanta(parsedHand: ParsedHand): YakuCheckResult {
		...
		const allMentsuHasYaochuuhai = parsedHand.mentsuList.every((mentsu) =>
			this.mentsuHasYaochuuhai(mentsu),
		);
		if (!allMentsuHasYaochuuhai) return null;

		const hasShuntsu = parsedHand.mentsuList.some(
	...
	public checkJyunchan(parsedHand: ParsedHand): YakuCheckResult {
		...
		const allMentsuHasRoutouhai = parsedHand.mentsuList.every((mentsu) =>
			this.mentsuHasRoutouhai(mentsu),
		);
