...
import {
	createDummyHand,
	createDummyParsedHand,
	createDummyYakuResult,
} from "@/utils/vitest/helper";
...
describe("FuCalculator", () => {
	...
	it("⭕️ 通常計算：メンゼン・ツモ (2符) のみ (20 + 2 = 22 -> 30符)", () => {
		const dummyParsedHand = createDummyParsedHand({
			...
			machiType: "ryanmen",
			janto: ["1m", "1m"],
			mentsuList: [new Mentsu("shuntsu", ["2m", "3m", "4m"], true)],
		});
	...
	it("⭕️ 通常計算：暗槓 (32符)", () => {
		const mentsuList = [new Mentsu("kantsu", ["1p", "1p", "1p", "1p"], true)];
	...
	it("⭕️ 通常計算：明槓（ミンカン）の符 (16符)", () => {
		const mentsuList = [new Mentsu("kantsu", ["1m", "1m", "1m", "1m"], false)];
		const dummyParsedHand = createDummyParsedHand({
			mentsuList: mentsuList,
			machiType: "ryanmen",
			janto: ["2p", "2p"],
