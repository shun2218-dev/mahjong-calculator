...
describe('PointCalculator', () => {
  ...
  describe('役満', () => {
    it('⭕️ 成立：役満（大三元）・子・ロン (32000点)', () => {
      ...
      const fu = 0;
      const dummyHand = createDummyHand({ agariType: 'ron' }, { jikaze: 'nan' });
    ...
    it('⭕️ 成立：ダブル役満（四暗刻単騎）・親・ツモ (16000点オール)', () => {
      ...
      const yakuResult = createDummyYakuResult(yakuList);
      const fu = 0;
      const dummyHand = createDummyHand({ agariType: 'tsumo' }, { jikaze: 'ton' });
    ...
    it('⭕️ 成立：数え役満 (13翻)・子・ロン (32000点)', () => {
      // 役満ではない役で 13翻
      const yakuList = [YAKU_LIST.CHINITSU, YAKU_LIST.RYANPEIKOU, YAKU_LIST.TANYAOCHUU, YAKU_LIST.RIICHI]; 
  ...
  describe('満貫〜三倍満', () => {
    it('⭕️ 成立：跳満 (6翻)・親・ロン (18000点)', () => {
      const yakuResult = createDummyYakuResult( [YAKU_LIST.CHINITSU, YAKU_LIST.TANYAOCHUU]);
      const fu = 30;
      const dummyHand = createDummyHand({ agariType: 'ron' }, { jikaze: 'ton' });
    ...
    it('⭕️ 成立：倍満 (8翻)・子・ツモ (4000/8000点)', () => {
      ...
      const fu = 30;
      const dummyHand = createDummyHand({ agariType: 'tsumo' }, { jikaze: 'nan' });
  ...
  describe('満貫未満', () => {
    it('⭕️ 成立：3翻30符・子・ロン (3900点)', () => {
      ...
      const yakuResult = createDummyYakuResult( [YAKU_LIST.RIICHI, YAKU_LIST.TANYAOCHUU, YAKU_LIST.PINFU]);
      const fu = 30;
      const dummyHand = createDummyHand({ agariType: 'ron' }, { jikaze: 'nan' }); // 子・ロン
    ...
    it('⭕️ 成立：2翻40符・子・ツモ (700/1300点)', () => {
      ...
      const yakuResult = createDummyYakuResult( [YAKU_LIST.TANYAOCHUU, YAKU_LIST.YAKUHAI_HAKU]);
      const fu = 40;
      const dummyHand = createDummyHand({ agariType: 'tsumo' }, { jikaze: 'nan' }); // 子・ツモ
    ...
    it('⭕️ 成立：4翻25符 (七対子)・親・ロン (9600点)', () => {
      ...
      const yakuResult = createDummyYakuResult( [YAKU_LIST.CHIITOITSU, YAKU_LIST.HONITSU]);
      const fu = 25;
      const dummyHand = createDummyHand({ agariType: 'ron' }, { jikaze: 'ton' }); // 親・ロン
    ...
    it('⭕️ 成立：切り上げ満貫 (3翻70符)・子・ロン (8000点)', () => {
      ...
      const yakuResult = createDummyYakuResult( [YAKU_LIST.TOITOIHOU, YAKU_LIST.YAKUHAI_HAKU]);
      const fu = 70;
      const dummyHand = createDummyHand({ agariType: 'ron' }, { jikaze: 'nan' }); // 子・ロン
