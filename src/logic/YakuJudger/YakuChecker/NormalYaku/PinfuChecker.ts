...
import type { Kazehai, Tile, YakuCheckResult } from "@/types";
...
export class PinfuChecker {
	public check(parsedHand: ParsedHand, hand: Hand): YakuCheckResult {
		...
		const isAllShuntsu = parsedHand.mentsuList.every(
		...
		if (
			this.isJantoYakuhai(jantoTile, hand.status.bakaze, hand.status.jikaze)
		) {
			return null;
		}
